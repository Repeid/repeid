...
import org.repeid.models.cache.infinispan.entities.AbstractRevisioned;
...
public abstract class CacheManager {
    ...
    public void addRevisioned(Revisioned object, long startupRevision) {
        ...
        try {
            ...
            Long rev = revisions.get(id);
            if (rev == null) {
                if (id.endsWith("realm.clients")) OrganizationCacheManager.logger.trace("addRevisioned rev == null realm.clients");
                rev = counter.current();
            ...
            if (rev == null) {
                if (id.endsWith("realm.clients")) OrganizationCacheManager.logger.trace("addRevisioned rev2 == null realm.clients");
                return;
            }
            if (rev > startupRevision) { // revision is ahead transaction start. Other transaction updated in the meantime. Don't cache
                if (OrganizationCacheManager.logger.isTraceEnabled()) {
                    OrganizationCacheManager.logger.tracev("Skipped cache. Current revision {0}, Transaction start revision {1}", object.getRevision(), startupRevision);
                }
                return;
            }
            if (rev.equals(object.getRevision())) {
                if (id.endsWith("realm.clients")) OrganizationCacheManager.logger.tracev("adding Object.revision {0} rev {1}", object.getRevision(), rev);
                cache.putForExternalRead(id, object);
                return;
            }
            if (rev > object.getRevision()) { // revision is ahead, don't cache
                if (id.endsWith("realm.clients")) OrganizationCacheManager.logger.trace("addRevisioned revision is ahead realm.clients");
                return;
            }
