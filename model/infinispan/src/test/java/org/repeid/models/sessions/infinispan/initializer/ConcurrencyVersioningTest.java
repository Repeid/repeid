...
public class ConcurrencyVersioningTest {
	...
	/**
	 * Test that if a put of an existing key is removed after the put and before
	 * tx commit, it is evicted
	 *
	 * @throws Exception
	 */
	@Test
	...
	protected DefaultCacheManager getVersionedCacheManager() {
		...
		invalidationConfigBuilder
				...
				.transaction().transactionMode(TransactionMode.TRANSACTIONAL).transaction()
				.transactionManagerLookup(new DummyTransactionManagerLookup()).locking()
				.isolationLevel(IsolationLevel.REPEATABLE_READ).writeSkewCheck(true).versioning().enable()
