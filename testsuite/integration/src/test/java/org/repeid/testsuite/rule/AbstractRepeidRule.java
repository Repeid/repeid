...
public abstract class AbstractRepeidRule extends ExternalResource {
    ...
    /*public void update(RepeidRule.RepeidSetup configurer, String realmId) {
        RepeidSession session = server.getSessionFactory().create();
        session.getTransaction().begin();
    ...
    public void stopSession(RepeidSession session, boolean commit) {
        RepeidTransaction transaction = session.getTransaction();
    ...
    public class DeploymentBuilder {
        ...
        //private Class<? extends RepeidConfigResolver> repeidConfigResolver;
        ...
        /*public DeploymentBuilder repeidConfigResolver(Class<? extends RepeidConfigResolver> repeidConfigResolver) {
            this.repeidConfigResolver = repeidConfigResolver;
            return this;
        }*/
        ...
        public void deployApplication() {
            ...
            /*if (null == repeidConfigResolver) {
                di.addInitParameter("repeid.config.file", adapterConfigPath);
            } else {
                di.addInitParameter("repeid.config.resolver", repeidConfigResolver.getCanonicalName());
            }*/
            if (isConstrained) {
                SecurityConstraint constraint = new SecurityConstraint();
                WebResourceCollection collection = new WebResourceCollection();
        ...
        public void deployApplicationWithFilter() {
            /*DeploymentInfo di = createDeploymentInfo(name, contextPath, servletClass);
            FilterInfo filter = new FilterInfo("repeid-filter", RepeidOIDCFilter.class);
            if (null == repeidConfigResolver) {
                filter.addInitParam("repeid.config.file", adapterConfigPath);
            } else {
                filter.addInitParam("repeid.config.resolver", repeidConfigResolver.getCanonicalName());
            }
            di.addFilter(filter);
            di.addFilterUrlMapping("repeid-filter", constraintUrl, DispatcherType.REQUEST);
            server.getServer().deploy(di);
*/
