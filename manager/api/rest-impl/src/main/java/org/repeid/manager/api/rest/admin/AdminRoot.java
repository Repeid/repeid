...
public class AdminRoot {
	...
	protected AdminAuth authenticateRealmAdminRequest(HttpHeaders headers) {
		...
		AuthenticationManager.AuthResult authResult = authManager.authenticateBearerToken(session, realm, uriInfo,
	...
	public RealmsAdminResource getRealmsAdmin(@Context final HttpHeaders headers) {
		...
		Cors.add(request).allowedOrigins(auth.getToken()).allowedMethods("GET", "PUT", "POST", "DELETE").auth()
				.build(response);
	...
	protected boolean isAdmin(AdminAuth auth) {
		...
		if (auth.getRealm().equals(realmManager.getKeycloakAdminstrationRealm())) {
			...
			ClientModel client = auth.getRealm()
