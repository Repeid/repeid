...
public class ApplianceBootstrap {
	...
	public boolean isNewInstall() {
		/*
		 * if (session.realms().getRealms().size() > 0) { return false; } else {
		 * return true; }
		 */
		return true;
	}
	...
	public boolean createMasterRealm(String contextPath) {
		 ...
		 * manager.setContextPath(contextPath); RealmModel realm =
		 * manager.createRealm(adminRealmName, adminRealmName);
		 * realm.setName(adminRealmName); realm.setDisplayName(Version.NAME);
		 * realm.setDisplayNameHtml(Version.NAME_HTML); realm.setEnabled(true);
		 * realm.addRequiredCredential(CredentialRepresentation.PASSWORD);
		 * realm.setSsoSessionIdleTimeout(1800);
		 * realm.setAccessTokenLifespan(60);
		 * realm.setAccessTokenLifespanForImplicitFlow(Constants.
		 * DEFAULT_ACCESS_TOKEN_LIFESPAN_FOR_IMPLICIT_FLOW_TIMEOUT);
		 * realm.setSsoSessionMaxLifespan(36000);
		 * realm.setOfflineSessionIdleTimeout(Constants.
	...
	public void createMasterRealmUser(String username, String password) {
		/*
		 * RealmModel realm = session.realms().getRealm(Config.getAdminRealm());
		 * if (session.users().getUsersCount(realm) > 0) { throw new
		 * IllegalStateException(
		 * "Can't create initial user as users already exists"); }
		 * 
