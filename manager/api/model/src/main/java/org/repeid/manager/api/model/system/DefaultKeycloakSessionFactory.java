...
import org.repeid.manager.api.model.provider.ProviderEvent;
import org.repeid.manager.api.model.provider.ProviderEventListener;
...
public class DefaultKeycloakSessionFactory implements RepeidSessionFactory {
		...
		for (Spi spi : ServiceLoader.load(Spi.class, getClass().getClassLoader())) {
			...
			if (provider != null) {
				...
				if (spi.isInternal() && !isInternal(factory)) {
					log.warnv(
							"{0} ({1}) is implementing the internal SPI {2}. This SPI is internal and may change without notice",
							factory.getId(), factory.getClass().getName(), spi.getName());
				}
