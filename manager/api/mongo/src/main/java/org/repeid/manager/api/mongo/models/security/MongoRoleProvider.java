...
import org.repeid.manager.api.mongo.models.AbstractHibernateStorage;
...
public class MongoRoleProvider extends AbstractHibernateStorage implements RoleProvider {
    ...
    @Override
    public RoleModel create(String name, String description, boolean autoGrant,
            Set<PermissionType> permissions, String createdBy) throws StorageException {
    ...
    public RoleModel findByName(String rolName) throws StorageException {
        TypedQuery<RoleEntity> query = getEntityManager().createNamedQuery("RoleEntity.findByName",
                RoleEntity.class);
        ...
        if (results.isEmpty()) {
            ...
            throw new IllegalStateException(
                    "Mas de un RoleEntity con nama=" + rolName + ", results=" + results);
        } else {
    ...
    public List<RoleModel> getAll(int firstResult, int maxResults) {
        TypedQuery<RoleEntity> query = getEntityManager().createNamedQuery("RoleEntity.findAll",
                RoleEntity.class);
        if (firstResult != -1) {
    ...
    public SearchResultsModel<RoleModel> search(SearchCriteriaModel criteria, String filterText) {
        SearchResultsModel<RoleEntity> entityResult = findFullText(criteria, RoleEntity.class, filterText,
                "name");
