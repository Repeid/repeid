...
import org.repeid.manager.api.model.utils.RepeidModelUtils;
import org.repeid.manager.api.model.utils.reflection.Property;
import org.repeid.manager.api.model.utils.reflection.PropertyQueries;
import org.repeid.manager.api.mongo.api.MongoCollection;
import org.repeid.manager.api.mongo.api.MongoEntity;
...
public class MongoStoreImpl implements MongoStore {
	...
	private static final Class<?>[] SIMPLE_TYPES = { String.class, Integer.class, Boolean.class, Long.class,
			Double.class, Character.class, Date.class, byte[].class };

	private final DB database;
	private static final Logger logger = Logger.getLogger(MongoStoreImpl.class);

	private final MapperRegistry mapperRegistry;
	private ConcurrentMap<Class<?>, EntityInfo> entityInfoCache = new ConcurrentHashMap<Class<?>, EntityInfo>();
	...
	public MongoStoreImpl(DB database, Class<?>[] managedEntityTypes) {
		...
		// Specific converter for ArrayList is added just for performance
		// purposes to avoid recursive converter lookup (most of list idm will
		// be ArrayList)
	...
	public void insertEntity(MongoIdentifiableEntity entity, MongoStoreInvocationContext context) {
		...
		// Find annotations for ID, for all the properties and for the name of
		// the collection.
		EntityInfo entityInfo = getEntityInfo(clazz);

		// Create instance of BasicDBObject and add all declared properties to
		// it (properties with null value probably should be skipped)
		...
		if (currentId == null) {
			currentId = RepeidModelUtils.generateId();
		...
		// Treat object as created in this transaction (It is already submitted
		// to transaction)
	...
	@Override
	public <T extends MongoIdentifiableEntity> int updateEntities(Class<T> type, DBObject query, DBObject update,
	...
	@Override
	public <T extends MongoIdentifiableEntity> T loadEntity(Class<T> type, String id,
			MongoStoreInvocationContext context) {
		// First look if we already read the object with this oid and type
		// during this transaction. If yes, use it instead of DB lookup
	...
	@Override
	public <T extends MongoIdentifiableEntity> T loadSingleEntity(Class<T> type, DBObject query,
			MongoStoreInvocationContext context) {
	...
	@Override
	public <T extends MongoIdentifiableEntity> List<T> loadEntities(Class<T> type, DBObject query,
			MongoStoreInvocationContext context) {
		// First we should execute all pending tasks before searching DB
		context.beforeDBSearch(type);
	...
	@Override
	public <T extends MongoIdentifiableEntity> List<T> loadEntities(Class<T> type, DBObject query, DBObject sort,
			int firstResult, int maxResults, MongoStoreInvocationContext context) {
		// First we should execute all pending tasks before searching DB
		context.beforeDBSearch(type);
	...
	public <T extends MongoIdentifiableEntity> int countEntities(Class<T> type, DBObject query,
			MongoStoreInvocationContext context) {
		context.beforeDBSearch(type);
	...
	@Override
	public boolean removeEntity(Class<? extends MongoIdentifiableEntity> type, String id,
			MongoStoreInvocationContext context) {
		...
		if (found == null) {
			...
			// logger.debugf("Entity of type: %s , id: %s removed from
			// MongoDB.", type, id);
	...
	@Override
	public int removeEntities(Class<? extends MongoIdentifiableEntity> type, DBObject query, boolean callback,
			MongoStoreInvocationContext context) {
		if (callback) {
	...
	@Override
	public <S> boolean pushItemToList(final MongoIdentifiableEntity entity, final String listPropertyName, S itemToPush,
			boolean skipIfAlreadyPresent, MongoStoreInvocationContext context) {
	...
	@Override
	public <S> boolean pullItemFromList(final MongoIdentifiableEntity entity, final String listPropertyName,
			final S itemToPull, MongoStoreInvocationContext context) {
		final Class<? extends MongoEntity> type = entity.getClass();
		EntityInfo entityInfo = getEntityInfo(type);
	...
	protected <T extends MongoIdentifiableEntity> List<T> convertCursor(Class<T> type, DBCursor cursor,
			MongoStoreInvocationContext context) {
	...
	protected <T extends MongoIdentifiableEntity> T convertDBObjectToEntity(Class<T> type, DBObject dbObject,
			MongoStoreInvocationContext context) {
		// First look if we already have loaded object cached. If yes, we will
		// use cached instance
