...
public class ClearExpiredEvents implements ScheduledTask {
	...
	public void run(KeycloakSession session) {
		 ...
		 * session.getProvider(EventStoreProvider.class); if (eventStore !=
		 * null) { for (RealmModel realm : session.realms().getRealms()) { if
		 * (realm.isEventsEnabled() && realm.getEventsExpiration() > 0) { long
