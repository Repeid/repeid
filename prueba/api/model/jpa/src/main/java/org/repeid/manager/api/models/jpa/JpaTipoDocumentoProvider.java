...
public class JpaTipoDocumentoProvider extends AbstractJpaStorage implements TipoDocumentoProvider {
	...
	public TipoDocumentoModel findByAbreviatura(String abreviatura) {
		...
		if (results.isEmpty()) {
			...
			throw new IllegalStateException(
					"Mas de un TipoDocumentoEntity con abreviatura=" + abreviatura + ", resultados=" + results);
		} else {
	...
	public boolean remove(TipoDocumentoModel tipoDocumento) {
		...
		TipoDocumentoEntity entity = em.find(TipoDocumentoEntity.class, tipoDocumento.getId());
		if (entity == null) {
			return false;
		}
		em.remove(entity);
	...
	public List<TipoDocumentoModel> getAll(int firstResult, int maxResults) {
		...
		entities.forEach(entity -> models.add(new TipoDocumentoAdapter(session, em, entity)));
		return models;
	}
	...
	public SearchResultsModel<TipoDocumentoModel> search(SearchCriteriaModel criteria) {
		...
		entityResult.getModels().forEach(entity -> list.add(new TipoDocumentoAdapter(session, em, entity)));
